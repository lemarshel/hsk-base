...
/* HanziWriter — wrap CJK chars in wordcell zh (lazy render) */
var hzPop=document.getElementById('hz-popup'),hzTimer=null;
function wrapHanziInScope(root){
  var scope = root || document;
  var list = scope.querySelectorAll('.wordcell .zh, .wc-inner .zh');
  list.forEach(function(div){
    if(div.dataset.hzWrapped) return;
    ...
    div.dataset.hzWrapped = '1';
  });
}

function ensureHanziWriter(cb){
  if(typeof HanziWriter!=='undefined' && HanziWriter){ cb(); return; }
  loadScriptOnce('hanzi-writer.min.js').then(function(){
    if(typeof HanziWriter!=='undefined' && HanziWriter) cb();
  }).catch(function(){});
}
...
function initSearchWorker(){
  ...
  var payload = rows.map(function(tr){
    var zhEl = tr.querySelector('.zh');
    var pyEl = tr.querySelector('.py');
    return {
      zh: zhEl ? zhEl.textContent.trim() : '',
      py: (tr.getAttribute('data-py') || (pyEl ? pyEl.textContent.trim() : '')),
...
(function(){
  ...
  function collectSection(h2){
    ...
    while(el && el.tagName !== 'H2'){
      if(el.tagName === 'H3' && el.classList.contains('phonetic-group')) info.nodes.push(el);
...
function rebuildView(){
  ...
  if(q){
    var effLang = lang;
    if(lang !== 'zh' && lang !== 'py' && lang !== 'en' && lang !== 'ru'){
      effLang = isEnUI ? 'en' : 'ru';
    }
    ...
    if(_searchWorkerReady){
      ...
      matched = allRows.filter(function(tr){
        ...
        var transTxt = '';
        if(effLang==='zh')      { return zhTxt.includes(qn); }
        else if(effLang==='py') { return pyTxt.includes(qn); }
        else if(effLang==='en') { transTxt = (tr.getAttribute('data-en')||'').toLowerCase(); }
...
function initVirtualTables(){
  ...
  ['size-zh','size-py','size-ru'].forEach(function(id){
    var el = document.getElementById(id);
