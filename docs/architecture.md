...
data/words.xlsx          ← YOU EDIT THIS
        │
        └─ python prod/make_test_page.py
                │
                ├─ test/index.html   (self-contained preview with debug banner)
                └─ index.html        (production — same pipeline, no banner)

data/groups.json         ← generated ONCE by prod/migrate.py from original HTML
        │
        └─ python prod/migrate.py  (re-run only if group structure changes)
                │
...
Both `test/index.html` and `index.html` are **fully self-contained** — all data
is inlined at build time, so they work when opened via `file://` with no server.

`test/index.html` has `<base href="../">` injected into its `<head>` so all
relative asset paths (`hsk.css`, `hsk.js`, etc.) resolve correctly from the
`test/` subdirectory.
...
```
hsk-base/
  index.html            ← production output (stays in root — GitHub Pages serves this)
  hsk.js                ← shared app logic
  hsk.css               ← shared styles
  hsk-head.js           ← shared
  hsk-body.js           ← shared
  sortable.min.js       ← shared lib
  hanzi-writer.min.js   ← shared lib
  text-topics.js        ← shared
  tracker.js            ← shared
  data/
    words.xlsx          ← THE ONLY FILE YOU EDIT
    groups.json         ← generated once by prod/migrate.py
    groups-data.js      ← generated once by prod/migrate.py
    words.json          ← generated by prod/xlsx_to_json.py (HTTP deployments)
    words-data.js       ← generated by prod/xlsx_to_json.py (HTTP deployments)
  js/
    render-words.js     ← builds word table DOM from HSK_WORDS + HSK_GROUPS
  prod/
    make_test_page.py   ← PRIMARY BUILD SCRIPT: reads xlsx → writes test/ and root index.html
    migrate.py          ← one-time migration tool (re-run only if group structure changes)
    xlsx_to_json.py     ← generates words-data.js for HTTP-served deployments
  test/
    index.html          ← test output with debug banner (auto-generated)
  docs/
...
4. Run:
   ...
   python prod/make_test_page.py
   ```
5. Refresh `test/index.html` in the browser — changes are visible immediately
...
Generated once by `prod/migrate.py` from `index.html`. Never hand-edited.
...
|---|---|---|
| `data/words.xlsx` | **YES — edit this** | `prod/migrate.py` (initial), then you |
| `data/words.json` | no | `prod/xlsx_to_json.py` |
| `data/words-data.js` | no | `prod/xlsx_to_json.py` |
| `data/groups.json` | no | `prod/migrate.py` |
| `data/groups-data.js` | no | `prod/migrate.py` |
| `js/render-words.js` | only if DOM shape changes | — |
| `hsk.js` | only for app logic changes | — |
| `hsk.css` | only for style changes | — |
| `prod/migrate.py` | only if migration logic changes | — |
| `prod/xlsx_to_json.py` | generates words-data.js for HTTP-served deployments | — |
| `prod/make_test_page.py` | only if index.html structure changes | — |
| `test/index.html` | no — rebuilt every run | `prod/make_test_page.py` |
| `index.html` | no — rebuilt every run | `prod/make_test_page.py` |
